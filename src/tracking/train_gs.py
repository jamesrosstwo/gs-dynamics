...
def train(seq, exp, remove_threshold, remove_thresh_5k, weight_soft_col_cons,
    ...
    out_path = ROOT_PATH / f"output/{exp}/{seq}"
    out_path.mkdir(parents=True)
    md_path = ROOT_PATH / f"data/{seq}/{metadata_path}"
    md = json.load(open(str(md_path), 'r'))  # metadata for custom dataset
        ...
        for i in range(num_iter_per_timestep):
            ...
            with torch.no_grad():
                ...
                if is_initial_timestep:
                    params, variables, num_pts = densify(params, variables, optimizer, i, remove_threshold,
                                                         remove_thresh_5k, scale_scene_radius)
                    num_pts_path = out_path / "num_pts.txt"
                    with open(num_pts_path, 'w') as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--remove_thresh_5k', type=float, default=0.25,
                        help='The threshold for removing points at 5k iterations.')
    parser.add_argument('--weight_soft_col_cons', type=float, default=0.01,
                        help='The weight for soft color consistency loss.')
    ...
    parser.add_argument('--num_knn', type=int, default=20,
                        help='The number of nearest neighbors to use for loss calculation.')
    ...
    parsed_weight_params = {f"weight_{k}": v for k, v in weight_params.items()}

    train(seq=args.sequence, exp=args.exp_name, remove_threshold=args.remove_threshold,
          remove_thresh_5k=args.remove_thresh_5k,
          **parsed_weight_params, num_knn=args.num_knn, scale_scene_radius=args.scale_scene_radius,
          metadata_path=args.metadata_path, init_pt_cld_path=args.init_pt_cld_path)
